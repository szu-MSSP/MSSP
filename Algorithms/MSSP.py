...
def load_or_build_A(graph_path, node_num, cache_dir):
    ...
    row, col = [], []
    with open(graph_path, "r") as file:
        for line in tqdm(file, desc="Reading edges for AT"):
            if line.startswith("%") or line.startswith("#"):
                continue
            from_node, to_node = map(int, line.strip().split()[:2])
    ...
    print("[NormCache] building AT ...", flush=True)
    S = A.sum(axis=0)
    S[S != 0] = 1.0 / S[S != 0]
...
def ensure_sampling_dist(graph_path, node_num, cache_dir, proc_dir, algo):
    ...
    if algo == "CUR":
        ...
        col_sums = np.asarray(A_sq.sum(axis=0)).ravel()
        row_sums = np.asarray(A_sq.sum(axis=1)).ravel()
        p_t2 = np.multiply(col_sums, row_sums)
        t2_p = p_t2 / p_t2.sum()
        np.save(t2p_path, t2_p.astype(np.float64))
        print(f"[DistCache] Saving", flush=True)
        del A_sq, col_sums, row_sums, p_t2, t2_p
        gc.collect()
        return t2p_path
...
def stage2_cpu_cur(proc_id, cpu_id, node_num, sampling_ratio, W, C, R):
    ...
    print(f"[proc{cpu_id}] iterations: {iter_count}, avg err: {final_avg_err:.18f}", flush=True)
    return R_cpu, svds_time, iter_time, t_svds_start, t_svds_end, t_iter_start, t_iter_end
...
def stage2_cpu_t2(proc_id, cpu_id, node_num, sampling_ratio, J, C, R):
    ...
    t_iter_end = time.perf_counter()
    iter_time = t_iter_end - t_iter_start
    print(f"[proc{cpu_id}] iterations: {iter_count}, avg err: {err / c_dim:.18f}", flush=True)
    return r_cpu, 0.0, iter_time, -1.0, -1.0, t_iter_start, t_iter_end



def worker_thread(proc_id,algo,node_num,A_csr,A_csc,row_p,col_p,t2_p,
    marks,res,tt,task_queue,error_list,):
    ...
    try:
        while True:
            ...
            if task is None:
                break
            job_id, sampling_ratio = task

            t_sample_start = time.perf_counter()
            c_total = int(node_num * sampling_ratio)
            c = (c_total * (proc_id + 1)) // num_threads - (c_total * proc_id) // num_threads
            if algo == "T2":
                sampled_index = rng.choice(node_num, size=c, replace=False, p=t2_p)
                sampled_index.sort()
                I = sampled_index
                J = sampled_index
            elif algo == "CUR":
                J = rng.choice(node_num, size=c, replace=False, p=col_p)
                I = rng.choice(node_num, size=c, replace=False, p=row_p)
                I.sort()
                J.sort()
            else:
            ...
            if algo == "CUR":
                R_full, svd_time, iter_time, t_svd_start, t_svd_end, t_iter_start, t_iter_end = (
...
def run_mthread(algo, graph_path, output_dir, node_num, ratio_list, cache_dir, proc_dir, num_threads):
    ...
    for job_id, sampling_ratio in enumerate(ratio_list):
        ...
        for _ in range(num_threads):
            task_queue.put((job_id, sampling_ratio))
        ...
        R_merge = np.mean(res, axis=0)
        R_merge /= np.linalg.norm(R_merge, ord=1)
        ...
        for i in range(num_threads):
            print(f"thread{i} weight                      : {1.0 / num_threads:.6f}", flush=True)
...
if __name__ == "__main__":
    ...
    ratio_list = [float(x) for x in args[8:]]
