...
def load_or_build_A(graph_path, node_num, cache_dir):
    ...
    if os.path.exists(A_path) and os.path.exists(csc_path) and os.path.exists(nodeids_path):
        ...
        print(f"[MapCache] Loading", flush=True)
...
def worker_thread(proc_id,algo,node_num,A_csr,A_csc,row_p,col_p,t2_p,
    marks,res,ww,ee,tt,task_queue,error_list,num_threads):
...
def run_mthread(algo, graph_path, output_dir, node_num, ratio_list, cache_dir, proc_dir, num_threads):
    ...
    ww = np.zeros(num_threads, dtype=np.float64)
    ...
    for job_id, sampling_ratio in enumerate(ratio_list):
        ...
        print(f"[Shuffle] ratio={sampling_ratio}, a={a}, b={b}",flush=True)
